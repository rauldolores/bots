...
export async function resolveAgentConfig(
  ...
  if (activeMcpConnectors.length > 0) {
    ...
    mcpToolsNote = `<herramientas_mcp>\nEste negocio tiene conectados estos sistemas externos:\n${lines.join(
      "\n",
    )}\n\nTus herramientas internas (captureLead, crear ticket, agendar cita) SOLO guardan la información dentro de Nodia Agents — NO la registran automáticamente en esos sistemas externos salvo que además llames la herramienta correspondiente de la lista de arriba. Cuando lo que estás haciendo tenga sentido en uno de esos sistemas (ej. registrar un lead o un cliente en un CRM, abrir un ticket en una plataforma de soporte), usa la herramienta que más se parezca a esa acción, ADEMÁS de tu herramienta interna — nunca asumas que una ya cubre a la otra.\n\nSi una de esas herramientas recibe SQL, NUNCA inventes nombres de tablas ni de columnas: llama primero a la herramienta de esquema del mismo sistema (la que termina en _get_schema) y usa SOLO lo que te devuelva. Pasó en produccion: el agente escribio un UPDATE contra columnas que no existian, el CRM lo rechazo y el cliente se quedo sin su comentario.\n\nNUNCA INVENTES UN ID. Si una herramienta pide el id de un contacto, una empresa, una oportunidad o un ticket y ese id no te lo dieron explícitamente en este prompt ni te lo devolvió una herramienta en esta misma conversación, BÚSCALO primero. Si la búsqueda no devuelve a nadie, dilo — no rellenes con un número. Pasó en producción: el agente puso id 1 por poner algo, el CRM rechazó la nota y la tarea por clave foránea, y el cliente colgó creyendo que las dos habían quedado.\n\nCÓMO SE BUSCA A UNA PERSONA: por su CORREO y, si no lo tienes, por su TELÉFONO — en ese orden, y solo por esos dos. NUNCA busques por nombre: estos sistemas no indexan nombres, así que buscar "Juan Pérez" devuelve "sin resultados" aunque Juan esté ahí, y te hará creer que no existe. Pasó: el cliente sí estaba en el CRM, se le buscó por su nombre, salió vacío, y el agente terminó inventando el id. Si tienes el correo o el teléfono de quien te escribe o te llama, úsalo para buscarlo ANTES de escribir cualquier cosa en esos sistemas.\n\nESTOS SISTEMAS TARDAN unos segundos en contestar. Antes de usar una de esas herramientas, escribe UNA frase corta diciendo qué vas a hacer ("Déjame registrar tu solicitud, un momento…", "Voy a revisar tu historial…"). Esa frase le llega al cliente de inmediato, mientras la consulta corre — sin ella el cliente ve el chat congelado y cree que lo dejaste plantado. Una sola frase, natural, y sigue con la herramienta.\n</herramientas_mcp>`;
  }
