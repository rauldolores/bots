...
//
//   npx nodia-agents init                 → asistente interactivo (elige idioma + licencia/gratis)
//   npx nodia-agents list                 → ver el catálogo
//   npx nodia-agents install <slug> [--key HZN-...]
//   npx nodia-agents update [dir] [--key HZN-...]   → jala la versión nueva
//   npx nodia-agents login                → conecta el CLI con tu cuenta (app.nodiagents.com)
//   npx nodia-agents pair [dir] --url …   → vincula un bot YA desplegado con tu dashboard
//
// Modo no-interactivo (para agentes/CI): pasa --yes y los datos por flags para que no
// se cuelgue esperando un menú. Ej:
//   npx nodia-agents init --yes --giro barberia --email tu@correo.com
//   npx nodia-agents install barberia --key HZN-XXXX-XXXX-XXXX --yes
...
// Las variables de entorno pasaron de FORJA_* a NODIA_*. Se leen LAS DOS: a
// quien ya tenía scripts o CI con las viejas no se le rompe nada, y no hay
// forma de que se entere de un cambio que no pidió.
function envCompat(nombre) {
  return process.env[`NODIA_${nombre}`] || process.env[`FORJA_${nombre}`];
}

const SERVER = envCompat("SERVER") || process.env.HORIZONTES_SERVER || "https://horizontes-license-server.innovandohorizontes.workers.dev";
const GET_URL = envCompat("GET_URL") || "https://horizontesia.com";
// Control plane de cuentas/pairing (app.nodiagents.com) — distinto del license server.
const CLOUD = envCompat("CLOUD") || "https://app.nodiagents.com";

// La config del usuario se mudó de ~/.forja a ~/.nodia. `migrarConfigVieja()`
// (abajo) copia lo que hubiera en la carpeta vieja la primera vez, así que nadie
// pierde su licencia ni su sesión al actualizar.
const CFG_DIR = join(homedir(), ".nodia");
const CFG_DIR_VIEJO = join(homedir(), ".forja");
const CFG_FILE = join(CFG_DIR, "config.json");
const CREDS_FILE = join(CFG_DIR, "credentials.json");

/**
 * Migra ~/.forja → ~/.nodia una sola vez, sin borrar la vieja.
 *
 * No se borra a propósito: si el usuario vuelve a una versión anterior del CLI,
 * su sesión sigue ahí. El costo de dejarla es una carpeta huérfana; el de
 * borrarla sería dejarlo sin licencia y sin explicación.
 */
function migrarConfigVieja() {
  try {
    if (existsSync(CFG_DIR) || !existsSync(CFG_DIR_VIEJO)) return;
    mkdirSync(CFG_DIR, { recursive: true });
    for (const archivo of ["config.json", "credentials.json"]) {
      const origen = join(CFG_DIR_VIEJO, archivo);
      if (existsSync(origen)) writeFileSync(join(CFG_DIR, archivo), readFileSync(origen));
    }
  } catch {
    // Que falle la migración no puede tumbar el CLI: en el peor caso el usuario
    // vuelve a conectarse.
  }
}
...
const DICT = {
  es: {
    ...
    optPaste: "Ya tengo mi licencia Nodia Agents+ (HZN-…)",
    ...
    promoActive: (d) => `Acceso FOUNDER activo · todo Nodia Agents+ hasta el ${d}`,
    optGet: "Conseguir Nodia Agents+ (los 14 giros y todo)",
    getTitle: "Nodia Agents+ · tu licencia viene con la comunidad",
    getBlurb: "Tu licencia se obtiene al entrar a la comunidad de Horizontes IA. Al unirte te llega tu LINK DE BIENVENIDA: activa Nodia Agents+ completo en tu panel y tu llave HZN-… sola, en un minuto (si ya la tienes, pégala aquí). Desbloqueas los 14 giros con su panel, los superpoderes y el Modo Agencia para revender.",
    ...
    step4: "al final: tu bot aparece en TU dashboard  →  https://app.nodiagents.com",
    welcomeTitle: "Bienvenido a Nodia Agents",
    welcomeBody: [
      ...
      "3) lo ves atender clientes desde tu dashboard → app.nodiagents.com",
    ],
    updateHint: "Actualiza cuando saquemos mejoras:  npx nodia-agents update",
    ...
    updTierUp: "⚡ Tu licencia es Nodia Agents+ — subí tu bot a PRO. Al desplegar, superpoderes prendidos.",
    ...
    commands: "Comandos:",
    // login / pair / whoami / logout (cuenta kontrolia.com)
    loginAlready: (e) => `ya estás conectado como ${e}`,
    loginForceHint: "para entrar con otra cuenta: npx nodia-agents login --force",
    ...
    loginRetryHint: "vuelve a intentarlo: npx nodia-agents login",
    loginOk: (e) => `conectado como ${e}`,
    licenseAdopted: (p) => `tu licencia Nodia Agents+ (${p}) quedó conectada — giros y updates premium desbloqueados`,
    loginPairHint: "siguiente paso (con tu bot ya desplegado): npx nodia-agents pair — lo conecta a tu dashboard",
    loginVerifyFail: "no pude verificar el token con el servidor. Vuelve a intentar: npx nodia-agents login",
    loginSavedUnverified: "sesión guardada, pero el servidor aún no la confirma. Pruébala en un momento: npx nodia-agents whoami",
    ...
    pageErrSub: "El enlace no es válido o expiró. Vuelve a la terminal y corre npx nodia-agents login otra vez.",
    pairNeedLogin: "no has iniciado sesión — corre: npx nodia-agents login",
    ...
    pairNeedsUpdate: "tu bot necesita actualizarse para conectarse al dashboard: npx nodia-agents update (y redespliega)",
    ...
    pairSessionExpired: "tu sesión expiró — corre: npx nodia-agents login",
    ...
    pairManual: [
      ...
      "  2. npx nodia-agents pair",
      "O ponlos a mano (los valores están en tu dashboard de kontrolia.com):",
    ...
    pairUnconfirmed: "los secretos quedaron puestos, pero el bot aún no confirma la conexión.\n    Tardan ~1 min en propagarse. Vuelve a probar: npx nodia-agents doctor",
    whoamiNone: "no has iniciado sesión — npx nodia-agents login",
    logoutOk: "sesión cerrada — se borró ~/.nodia/credentials.json",
    logoutNone: "no había sesión guardada.",
    netFail: (u) => `no pude contactar ${u} — revisa tu internet e inténtalo de nuevo.`,
    helpAccount: "Cuenta kontrolia.com:  login (abre el navegador) · pair (vincula tu bot desplegado) · whoami · logout",
  },
  en: {
    ...
    optPaste: "I already have my Nodia Agents+ license (HZN-…)",
    ...
    promoActive: (d) => `FOUNDER access active · full Nodia Agents+ until ${d}`,
    optGet: "Get Nodia Agents+ (all 14 niches and more)",
    getTitle: "Nodia Agents+ · your license comes with the community",
    ...
    step4: "at the end: your bot shows up in YOUR dashboard  →  https://app.nodiagents.com",
    welcomeTitle: "Welcome to Nodia Agents",
    welcomeBody: [
      ...
      "3) watch it serve customers from your dashboard → app.nodiagents.com",
    ],
    updateHint: "Update whenever we ship improvements:  npx nodia-agents update",
    ...
    updTierUp: "⚡ Your license is Nodia Agents+ — bumped your bot to PRO. Deploy and the superpowers are on.",
    ...
    commands: "Commands:",
    // login / pair / whoami / logout (kontrolia.com account)
    loginAlready: (e) => `already logged in as ${e}`,
    loginForceHint: "to switch accounts: npx nodia-agents login --force",
    ...
    loginRetryHint: "try again: npx nodia-agents login",
    loginOk: (e) => `logged in as ${e}`,
    licenseAdopted: (p) => `your Nodia Agents+ license (${p}) is now connected — premium niches and updates unlocked`,
    loginPairHint: "next step (with your bot deployed): npx nodia-agents pair — links it to your dashboard",
    loginVerifyFail: "couldn't verify the token with the server. Try again: npx nodia-agents login",
    loginSavedUnverified: "session saved, but the server hasn't confirmed it yet. Try in a moment: npx nodia-agents whoami",
    ...
    pageErrSub: "The link is invalid or expired. Go back to the terminal and run npx nodia-agents login again.",
    pairNeedLogin: "you're not logged in — run: npx nodia-agents login",
    ...
    pairNeedsUpdate: "your bot needs an update to connect to the dashboard: npx nodia-agents update (then redeploy)",
    ...
    pairSessionExpired: "your session expired — run: npx nodia-agents login",
    ...
    pairManual: [
      ...
      "  2. npx nodia-agents pair",
      "Or set them manually (values live in your kontrolia.com dashboard):",
      "  npx wrangler secret put CONTROL_PLANE_TOKEN",
      "  npx wrangler secret put CONTROL_PLANE_URL",
    ],
    pairVerifying: "Confirming the connection with the bot… ",
    pairOk: (u) => `bot connected — see it at ${u}`,
    pairUnconfirmed: "secrets are set, but the bot hasn't confirmed the connection yet.\n    They take ~1 min to propagate. Try again: npx nodia-agents doctor",
    whoamiNone: "not logged in — npx nodia-agents login",
    logoutOk: "logged out — ~/.nodia/credentials.json deleted",
    logoutNone: "there was no saved session.",
    netFail: (u) => `couldn't reach ${u} — check your internet and try again.`,
    helpAccount: "kontrolia.com account:  login (opens the browser) · pair (links your deployed bot) · whoami · logout",
  },
};
let L = "es";
const t = () => DICT[L];
...
// terminal interactiva. `interactive()` es false si no hay TTY o si se pasó --yes
// (o NODIA_YES / FORJA_YES).
...
function loadCfg() { migrarConfigVieja(); try { return JSON.parse(readFileSync(CFG_FILE, "utf8")); } catch { return {}; } }
...
// ── banner ──────────────────────────────────────────────────────────────────
// El wordmark se rehace con cada cambio de marca. NODIA cabe en 34 columnas,
// así que entra en cualquier terminal sin partirse.
...
const WORDMARK_GRAD = [39, 45, 51, 45, 39];
function forgeSplash() {
  const noColor = process.env.NO_COLOR || envCompat("NO_ART");
  if (noColor) { console.log("\n  " + C.b("◇ NODIA AGENTS") + "\n"); return; }
  const out = ["", c256(45, "   · ˚ ✦ ˖ ✧")];
  WORDMARK.forEach((l, i) => out.push("  " + c256(WORDMARK_GRAD[i], l)));
  out.push(c256(24, "   ▂▃▄▅▆▇█ bots que atienden por ti █▇▆▅▄▃▂"), "");
  console.log(out.join("\n"));
}

function banner() { console.log(C.cyan("\n  ◇ Nodia Agents") + C.dim("  ·  " + t().tagline + "\n")); }
...
function stampBotConfig(dir, plan, slug) {
  ...
  // "starter" gratis) tomarían el MISMO worker y se pisarían el nombre. Si el
  // toml ya trae un uid stampeado (reinstalación en la misma carpeta) se REUSA;
  // si es el del demo/placeholder, se genera uno nuevo. `update` excluye
  // wrangler.toml, así que el uid persiste entre actualizaciones.
  //
  // Se acepta el prefijo VIEJO (`forja-`) además del nuevo: un bot ya desplegado
  // tiene que conservar su nombre de worker, o el siguiente deploy crearía uno
  // nuevo y abandonaría el que está atendiendo clientes.
  const existingUid = (s.match(/name\s*=\s*"(?:nodia|forja)-.+-([a-f0-9]{6})"/) || [])[1];
  const existingName = (s.match(/^name\s*=\s*"((?:nodia|forja)-[^"]+)"/m) || [])[1];
  ...
  // así que va vacía: `nodia-agents pair` la escribe tras el deploy y el runtime cae a su
  // propio origin si está vacía (ver selfOrigin en el template) — sin edición manual.
  // Un bot ya bautizado conserva su nombre; uno nuevo estrena prefijo.
  s = s.replace(/^name\s*=\s*"[^"]+"/m, `name = "${existingName || `nodia-${safeSlug}-${botUid}`}"`);
  s = s.replace(/DASHBOARD_BASE_URL\s*=\s*"[^"]*"/g, `DASHBOARD_BASE_URL = ""`);
  // Ya no se sellan recursos de Cloudflare: los datos y la búsqueda vectorial
  // viven en la Supabase del usuario, a la que se llega con el secreto
  // DATABASE_URL. Cada bot necesita su PROPIA base (o su propio esquema): si dos
  // comparten, mezclarían conversaciones y base de conocimiento entre negocios.
  writeFileSync(wt, s);
}
...
// del miembro a "pro" (el camino de upgrade tras activar Nodia Agents+): update --key
...
async function pickBot(bots, userPlan, rl, wantSlug) {
  ...
  if (!interactive()) {
    ...
    agentBriefing(
      ...
      "npx nodia-agents init --yes --giro <slug>",
    );
    process.exit(1);
  }
...
async function cmdInit(flags = {}) {
  ...
  try {
      ...
      if (!EMAIL_RE.test(email) || !code) {
        if (!interactive()) {
          agentBriefing(
            ...
            'npx nodia-agents init --yes --codigo <CODIGO> --email <correo> --name "<nombre>" [--giro <slug>]',
          );
          process.exit(1);
        }
    ...
    if (!key && eventCode) {
      ...
      // "guardada" solo si viene de ~/.nodia; si viene del flag, aún no está validada ni guardada.
      ...
      // El correo NO se pide aquí: es opt-in al FINAL (con el pitch de Nodia Agents+, lo hace el skill
      // con `nodia-agents suscribir`). Quien ya tiene Nodia Agents+ pasa --key; código de evento pasa --codigo.
    ...
    // Bienvenida con contexto ANTES de las preguntas: qué es Nodia Agents, dónde vive el
...
async function cmdUpdate(dirArg, flags) {
  ...
  console.log(C.dim("    ") + C.cyan(t().updPublishCmd) + C.dim("  (npm install && npm run deploy)\n"));
}

// doctor — diagnostica el bot instalado: config local, versión, licencia y si el
// worker responde. Uso recurrente: corre `npx nodia-agents doctor` cuando algo falle.
async function cmdDoctor(dirArg, flags) {
  ...
  const dir = resolveBotDir(dirArg);
  if (!dir) { bad("No encontré un bot aquí.", "Corre esto dentro de la carpeta de tu bot, o pásala: nodia-agents doctor <carpeta>"); process.exit(1); }
  ...
  if (has("node_modules")) ok("Dependencias instaladas"); else warn("Dependencias sin instalar", "Corre: npm install");
  if (has(join("member", "config.local.ts"))) ok("Negocio configurado (member/config.local.ts)"); else warn("El negocio aún no está configurado", "Corre el onboarding: nodia-agents init");
  ...
  try {
    ...
    if (bot && marker.version) {
      if (verLt(marker.version, bot.version)) warn(`Hay una versión nueva: v${bot.version} (tienes v${marker.version})`, "Actualiza: nodia-agents update");
  ...
  } else warn("Sin licencia guardada", "Necesaria para actualizar. Pásala: nodia-agents doctor --key HZN-...");
  ...
  if (baseUrl) {
    ...
    } catch { warn("El bot no respondió", `¿Ya desplegaste? npm run deploy · URL: ${baseUrl}`); }
  } else warn("Sin DASHBOARD_BASE_URL", "No pude probar si el bot está en línea; se llena al desplegar.");

  // 7) pairing con el dashboard de kontrolia.com (solo si `pair` ya corrió)
  if (marker.paired) {
    if (baseUrl) {
      try {
        ...
        if (r.status === 200 || r.status === 401) ok("Dashboard kontrolia.com: conectado");
        else if (r.status === 404) warn("Dashboard kontrolia.com: el bot no trae la API de pairing", "Actualiza y reconecta: npx nodia-agents update · npx nodia-agents pair");
        else warn(`Dashboard kontrolia.com: el bot respondió HTTP ${r.status}`, "Reintenta: npx nodia-agents pair");
      } catch { warn("Dashboard kontrolia.com: el bot no respondió", "¿Sigue desplegado? Reintenta: npx nodia-agents pair"); }
    } else warn("Dashboard kontrolia.com: vinculado pero sin DASHBOARD_BASE_URL", "No pude probar la conexión.");
  }
...
// ── cuenta kontrolia.com: login / pair / whoami / logout ────────────────────
// El CLI se vincula a la cuenta del usuario en app.nodiagents.com (estilo
...
function loadCreds() { migrarConfigVieja(); try { return JSON.parse(readFileSync(CREDS_FILE, "utf8")); } catch { return null; } }
...
// Página que ve el navegador al terminar (autocontenida).
...
<title>nodia-agents login</title>
<body style="margin:0;min-height:100vh;display:grid;place-items:center;background:#141009;color:#efe7da;font-family:'JetBrains Mono',ui-monospace,monospace">
  <main style="background:#1d1710;border:1px solid #352a1d;box-shadow:8px 8px 0 #4c3a26;padding:32px 36px;max-width:380px">
    <p style="margin:0;font-size:10px;text-transform:uppercase;letter-spacing:.22em;color:#726555">kontrolia bots &middot; cli</p>
...
async function cmdLogin(flags = {}) {
  ...
  if (!flags["no-browser"] && !envCompat("NO_BROWSER")) openBrowser(authUrl);
...
// regresa una licencia (cuenta Nodia Agents+ activada en /bienvenida), se adopta SOLA
// en ~/.nodia/config.json — cero llaves tecleadas. Los giros y updates premium
...
async function cmdPair(dirArg, flags = {}) {
  ...
  if (!creds?.token) {
    ...
    if (!interactive()) agentBriefing(
      ["Corre `npx nodia-agents login` primero — imprime una URL; pásasela al usuario para que la abra en su navegador y autorice."],
      "npx nodia-agents login && npx nodia-agents pair --url https://<worker>.workers.dev",
    );
    process.exit(1);
  }
  ...
  if (!rawUrl) {
    ...
    agentBriefing(
      ...
      "npx nodia-agents pair --url https://<worker>.workers.dev",
    );
    process.exit(1);
  }
...
async function cmdWhoami() {
  ...
  try {
    ...
    if (me.status === 200 && me.ok) {
      ...
      adoptLicense(me);   // si activó Nodia Agents+ después del login, aquí se adopta
      console.log("");
      return;
    }
...
// Se ejecuta como CLI solo cuando se invoca directo (npx nodia-agents / node cli.js), no
// cuando se importa para pruebas (ahí solo se exponen las funciones puras de abajo).
// Robusto ante symlinks: npx expone el bin como enlace "nodia-agents" (no "cli.js"), así
...
const IS_MAIN = (() => {
  ...
  return base === "cli.js" || base === "nodia-agents";
})();
...
function cmdAyuda() {
  ...
  banner();
  if (L === "en") {
    console.log("  " + C.b("🆘 Nodia Agents help & support") + "\n");
    console.log("  " + C.cyan("📚 Docs & guides") + "     https://nodiagents.com/docs");
    console.log("  " + C.cyan("🎬 Video tutorials") + "   https://nodiagents.com/docs · connections, install and more");
    ...
    console.log("   3. The output of " + C.cyan("npx nodia-agents doctor") + " (run it inside the bot folder)\n");
    console.log(C.dim("  License issues (expired, event code, install limit): same DM.\n"));
  } else {
    console.log("  " + C.b("🆘 Ayuda y soporte de Nodia Agents") + "\n");
    console.log("  " + C.cyan("📚 Guías y docs") + "      https://nodiagents.com/docs");
    console.log("  " + C.cyan("🎬 Videotutoriales") + "   https://nodiagents.com/docs · conexiones, instalación y más");
    ...
    console.log("   3. Lo que dice " + C.cyan("npx nodia-agents doctor") + " (córrelo en la carpeta del bot)\n");
