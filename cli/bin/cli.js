...
//
//   npx kontrolia-bots init                 → asistente interactivo (elige idioma + licencia/gratis)
//   npx kontrolia-bots list                 → ver el catálogo
//   npx kontrolia-bots install <slug> [--key HZN-...]
//   npx kontrolia-bots update [dir] [--key HZN-...]   → jala la versión nueva
//   npx kontrolia-bots login                → conecta el CLI con tu cuenta (app.kontrolia.com)
//   npx kontrolia-bots pair [dir] --url …   → vincula un bot YA desplegado con tu dashboard
//
// Modo no-interactivo (para agentes/CI): pasa --yes y los datos por flags para que no
// se cuelgue esperando un menú. Ej:
//   npx kontrolia-bots init --yes --giro barberia --email tu@correo.com
//   npx kontrolia-bots install barberia --key HZN-XXXX-XXXX-XXXX --yes
...
// Las variables de entorno pasaron de FORJA_* a KONTROLIA_*. Se leen LAS DOS: a
...
function envCompat(nombre) {
  return process.env[`KONTROLIA_${nombre}`] || process.env[`FORJA_${nombre}`];
}
...
// Control plane de cuentas/pairing (app.kontrolia.com) — distinto del license server.
const CLOUD = envCompat("CLOUD") || "https://app.kontrolia.com";

// La config del usuario se mudó de ~/.forja a ~/.kontrolia. `migrarConfigVieja()`
// (abajo) copia lo que hubiera en la carpeta vieja la primera vez, así que nadie
// pierde su licencia ni su sesión al actualizar.
const CFG_DIR = join(homedir(), ".kontrolia");
...
/**
 * Migra ~/.forja → ~/.kontrolia una sola vez, sin borrar la vieja.
 *
...
const DICT = {
  es: {
    ...
    optPaste: "Ya tengo mi licencia KontrolIA Bots+ (HZN-…)",
    ...
    promoActive: (d) => `Acceso FOUNDER activo · todo KontrolIA Bots+ hasta el ${d}`,
    optGet: "Conseguir KontrolIA Bots+ (los 14 giros y todo)",
    getTitle: "KontrolIA Bots+ · tu licencia viene con la comunidad",
    getBlurb: "Tu licencia se obtiene al entrar a la comunidad de Horizontes IA. Al unirte te llega tu LINK DE BIENVENIDA: activa KontrolIA Bots+ completo en tu panel y tu llave HZN-… sola, en un minuto (si ya la tienes, pégala aquí). Desbloqueas los 14 giros con su panel, los superpoderes y el Modo Agencia para revender.",
    ...
    step4: "al final: tu bot aparece en TU dashboard  →  https://app.kontrolia.com",
    welcomeTitle: "Bienvenido a KontrolIA Bots",
    welcomeBody: [
      ...
      "3) lo ves atender clientes desde tu dashboard → app.kontrolia.com",
    ],
    updateHint: "Actualiza cuando saquemos mejoras:  npx kontrolia-bots update",
    ...
    updTierUp: "⚡ Tu licencia es KontrolIA Bots+ — subí tu bot a PRO. Al desplegar, superpoderes prendidos.",
    ...
    loginForceHint: "para entrar con otra cuenta: npx kontrolia-bots login --force",
    ...
    loginRetryHint: "vuelve a intentarlo: npx kontrolia-bots login",
    loginOk: (e) => `conectado como ${e}`,
    licenseAdopted: (p) => `tu licencia KontrolIA Bots+ (${p}) quedó conectada — giros y updates premium desbloqueados`,
    loginPairHint: "siguiente paso (con tu bot ya desplegado): npx kontrolia-bots pair — lo conecta a tu dashboard",
    loginVerifyFail: "no pude verificar el token con el servidor. Vuelve a intentar: npx kontrolia-bots login",
    loginSavedUnverified: "sesión guardada, pero el servidor aún no la confirma. Pruébala en un momento: npx kontrolia-bots whoami",
    ...
    pageErrSub: "El enlace no es válido o expiró. Vuelve a la terminal y corre npx kontrolia-bots login otra vez.",
    pairNeedLogin: "no has iniciado sesión — corre: npx kontrolia-bots login",
    ...
    pairNeedsUpdate: "tu bot necesita actualizarse para conectarse al dashboard: npx kontrolia-bots update (y redespliega)",
    ...
    pairSessionExpired: "tu sesión expiró — corre: npx kontrolia-bots login",
    ...
    pairUnconfirmed: "los secretos quedaron puestos, pero el bot aún no confirma la conexión.\n    Tardan ~1 min en propagarse. Vuelve a probar: npx kontrolia-bots doctor",
    whoamiNone: "no has iniciado sesión — npx kontrolia-bots login",
    logoutOk: "sesión cerrada — se borró ~/.kontrolia/credentials.json",
  ...
  en: {
    ...
    optPaste: "I already have my KontrolIA Bots+ license (HZN-…)",
    ...
    promoActive: (d) => `FOUNDER access active · full KontrolIA Bots+ until ${d}`,
    optGet: "Get KontrolIA Bots+ (all 14 niches and more)",
    getTitle: "KontrolIA Bots+ · your license comes with the community",
    ...
    step4: "at the end: your bot shows up in YOUR dashboard  →  https://app.kontrolia.com",
    welcomeTitle: "Welcome to KontrolIA Bots",
    welcomeBody: [
      ...
      "3) watch it serve customers from your dashboard → app.kontrolia.com",
    ],
    updateHint: "Update whenever we ship improvements:  npx kontrolia-bots update",
    ...
    updTierUp: "⚡ Your license is KontrolIA Bots+ — bumped your bot to PRO. Deploy and the superpowers are on.",
    ...
    loginForceHint: "to switch accounts: npx kontrolia-bots login --force",
    ...
    loginRetryHint: "try again: npx kontrolia-bots login",
    loginOk: (e) => `logged in as ${e}`,
    licenseAdopted: (p) => `your KontrolIA Bots+ license (${p}) is now connected — premium niches and updates unlocked`,
    loginPairHint: "next step (with your bot deployed): npx kontrolia-bots pair — links it to your dashboard",
    loginVerifyFail: "couldn't verify the token with the server. Try again: npx kontrolia-bots login",
    loginSavedUnverified: "session saved, but the server hasn't confirmed it yet. Try in a moment: npx kontrolia-bots whoami",
    ...
    pageErrSub: "The link is invalid or expired. Go back to the terminal and run npx kontrolia-bots login again.",
    pairNeedLogin: "you're not logged in — run: npx kontrolia-bots login",
    ...
    pairNeedsUpdate: "your bot needs an update to connect to the dashboard: npx kontrolia-bots update (then redeploy)",
    ...
    pairSessionExpired: "your session expired — run: npx kontrolia-bots login",
    ...
    pairUnconfirmed: "secrets are set, but the bot hasn't confirmed the connection yet.\n    They take ~1 min to propagate. Try again: npx kontrolia-bots doctor",
    whoamiNone: "not logged in — npx kontrolia-bots login",
    logoutOk: "logged out — ~/.kontrolia/credentials.json deleted",
...
// ── banner ──────────────────────────────────────────────────────────────────
// El arte decía "FORJA" y la marca ya no se llama así. Se rehízo en 62
// columnas para que quepa en cualquier terminal sin partirse.
...
function stampBotConfig(dir, plan, slug) {
  ...
  const existingUid = (s.match(/name\s*=\s*"(?:kontrolia|forja)-.+-([a-f0-9]{6})"/) || [])[1];
  const existingName = (s.match(/^name\s*=\s*"((?:kontrolia|forja)-[^"]+)"/m) || [])[1];
  ...
  // así que va vacía: `kontrolia-bots pair` la escribe tras el deploy y el runtime cae a su
  ...
  s = s.replace(/^name\s*=\s*"[^"]+"/m, `name = "${existingName || `kontrolia-${safeSlug}-${botUid}`}"`);
...
// del miembro a "pro" (el camino de upgrade tras activar KontrolIA Bots+): update --key
...
async function pickBot(bots, userPlan, rl, wantSlug) {
  ...
  if (!interactive()) {
    ...
    agentBriefing(
      ...
      "npx kontrolia-bots init --yes --giro <slug>",
    );
    process.exit(1);
  }
...
async function cmdInit(flags = {}) {
  ...
  try {
      ...
      if (!EMAIL_RE.test(email) || !code) {
        if (!interactive()) {
          agentBriefing(
            ...
            'npx kontrolia-bots init --yes --codigo <CODIGO> --email <correo> --name "<nombre>" [--giro <slug>]',
          );
          process.exit(1);
        }
    ...
    if (!key && eventCode) {
      ...
      // "guardada" solo si viene de ~/.kontrolia; si viene del flag, aún no está validada ni guardada.
      ...
      // El correo NO se pide aquí: es opt-in al FINAL (con el pitch de KontrolIA Bots+, lo hace el skill
      // con `kontrolia-bots suscribir`). Quien ya tiene KontrolIA Bots+ pasa --key; código de evento pasa --codigo.
    ...
    // Bienvenida con contexto ANTES de las preguntas: qué es KontrolIA Bots, dónde vive el
...
// worker responde. Uso recurrente: corre `npx kontrolia-bots doctor` cuando algo falle.
async function cmdDoctor(dirArg, flags) {
  ...
  const dir = resolveBotDir(dirArg);
  if (!dir) { bad("No encontré un bot aquí.", "Corre esto dentro de la carpeta de tu bot, o pásala: kontrolia-bots doctor <carpeta>"); process.exit(1); }
  ...
  if (has(join("member", "config.local.ts"))) ok("Negocio configurado (member/config.local.ts)"); else warn("El negocio aún no está configurado", "Corre el onboarding: kontrolia-bots init");
  ...
  try {
    ...
    if (bot && marker.version) {
      if (verLt(marker.version, bot.version)) warn(`Hay una versión nueva: v${bot.version} (tienes v${marker.version})`, "Actualiza: kontrolia-bots update");
  ...
  } else warn("Sin licencia guardada", "Necesaria para actualizar. Pásala: kontrolia-bots doctor --key HZN-...");
  ...
  if (marker.paired) {
    if (baseUrl) {
      try {
        ...
        else if (r.status === 404) warn("Dashboard kontrolia.com: el bot no trae la API de pairing", "Actualiza y reconecta: npx kontrolia-bots update · npx kontrolia-bots pair");
        else warn(`Dashboard kontrolia.com: el bot respondió HTTP ${r.status}`, "Reintenta: npx kontrolia-bots pair");
      } catch { warn("Dashboard kontrolia.com: el bot no respondió", "¿Sigue desplegado? Reintenta: npx kontrolia-bots pair"); }
...
// El CLI se vincula a la cuenta del usuario en app.kontrolia.com (estilo
...
<title>kontrolia-bots login</title>
...
// regresa una licencia (cuenta KontrolIA Bots+ activada en /bienvenida), se adopta SOLA
// en ~/.kontrolia/config.json — cero llaves tecleadas. Los giros y updates premium
...
async function cmdPair(dirArg, flags = {}) {
  ...
  if (!creds?.token) {
    ...
    if (!interactive()) agentBriefing(
      ["Corre `npx kontrolia-bots login` primero — imprime una URL; pásasela al usuario para que la abra en su navegador y autorice."],
      "npx kontrolia-bots login && npx kontrolia-bots pair --url https://<worker>.workers.dev",
    );
    process.exit(1);
  }
  ...
  if (!rawUrl) {
    ...
    agentBriefing(
      ...
      "npx kontrolia-bots pair --url https://<worker>.workers.dev",
    );
    process.exit(1);
  }
...
async function cmdWhoami() {
  ...
  try {
    ...
    if (me.status === 200 && me.ok) {
      ...
      adoptLicense(me);   // si activó KontrolIA Bots+ después del login, aquí se adopta
      console.log("");
      return;
    }
...
// Se ejecuta como CLI solo cuando se invoca directo (npx kontrolia-bots / node cli.js), no
// cuando se importa para pruebas (ahí solo se exponen las funciones puras de abajo).
// Robusto ante symlinks: npx expone el bin como enlace "kontrolia-bots" (no "cli.js"), así
...
const IS_MAIN = (() => {
  ...
  return base === "cli.js" || base === "kontrolia-bots";
})();
...
function cmdAyuda() {
  ...
  banner();
  if (L === "en") {
    console.log("  " + C.b("🆘 KontrolIA Bots help & support") + "\n");
    ...
    console.log("   3. The output of " + C.cyan("npx kontrolia-bots doctor") + " (run it inside the bot folder)\n");
    console.log(C.dim("  License issues (expired, event code, install limit): same DM.\n"));
  } else {
    console.log("  " + C.b("🆘 Ayuda y soporte de KontrolIA Bots") + "\n");
    ...
    console.log("   3. Lo que dice " + C.cyan("npx kontrolia-bots doctor") + " (córrelo en la carpeta del bot)\n");
